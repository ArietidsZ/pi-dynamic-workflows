...
- **Measured usage** — report real tokens and cost from each subagent session; add run, phase, or agent budgets only when you want them. When a provider session ends without reporting usage, the affected totals are heuristic character estimates and render with a `~` prefix (e.g. `~640 tok`) on every surface — never silently as metered figures.
...
| `budget` | Inspect tokens spent and remaining (`~` marks heuristic estimates) |
